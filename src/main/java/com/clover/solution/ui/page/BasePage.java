...
public class BasePage{
  ...
  //Navigate Service
    protected void navigateToIFrame(WebElement webElement) {
    	try {
    		driver.switchTo().frame(webElement);
    		ExtentTestManager.getTest().log(Status.PASS, "NavigateToURL - Navigated to URL - " + webElement);
    	}catch(Exception e)
    	{
    		ExtentTestManager.getTest().log(Status.FAIL, classname + "-Exception in navigateToURL" + e.getMessage());
    		Log.error(classname + "-Exception in navigateToURL" + e.getMessage());
  ...
  //Navigate Service
    protected void EscapeFromIFrame() {
    	try {
    		driver.switchTo().defaultContent();
    		ExtentTestManager.getTest().log(Status.PASS, "EscapeFromIFrame");
    	}catch(Exception e)
    	{
    		ExtentTestManager.getTest().log(Status.FAIL, classname + "-Exception in EscapeFromIFrame" + e.getMessage());
    		Log.error(classname + "-Exception in EscapeFromIFrame" + e.getMessage());
    	}
    }
  ...
  //clickTheElement
	...
	protected boolean assertPageContentContains(String expected, String Actual)
	{

		Log.info("Actual: Actual - " + Actual );
		Log.info("Expected: Expected - " + expected );
		if(Actual.contains(expected)) {
			ExtentTestManager.getTest().log(Status.PASS, "AssertPageTitle: Actual-" + Actual + " with Expected-" + expected );
			Assert.assertTrue(true);
			return true;
		}
		else {
			ExtentTestManager.getTest().log(Status.FAIL, "AssertPageTitle: Actual-" + Actual + " with Expected-" + expected );
			Assert.assertTrue(false);
			return false;
		}
	}
